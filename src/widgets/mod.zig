...
//!
//! For most uses, prefer the high-level components in `gooey-components`:
//!
//! ```zig
...
//! components.TextInput{ .id = "name", .placeholder = "Enter name" }
//! components.TextArea{ .id = "bio", .placeholder = "Enter bio" }
//!
...
// component is `components.TextInput` in `components/src/text_input.zig`.
...
// `TextAreaState` is the heap-allocated engine; `components.TextArea` in
// `components/src/text_area.zig` is the user-facing declarative component.
