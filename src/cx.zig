...
pub const Cx = struct {
    ...
    ///
    /// Components resolve identity once and reuse it for accessibility and
    /// `ui.box_with_layout_id`. When `id` is null, this calls `generateId` while
    /// the parent is still open so identity remains parent-scoped.
